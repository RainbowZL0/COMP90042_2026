...
The first stage uses sparse lexical retrieval. We compare TF-IDF and the BM25 ranking function \citep{robertson2009probabilistic}, implemented with the Python \texttt{bm25s} library, with three preprocessing settings: lowercasing only, lowercasing plus stopword removal, and lowercasing plus stopword removal plus Porter stemming \citep{porter1980algorithm}. We choose the first-stage configuration by development recall@500, because evidence that is absent from the top-500 pool cannot be recovered by downstream reranking.
...
Table~\ref{tab:search-space} summarises the main search space. The search is organised around the role of each module rather than one single metric. For the first-stage retriever, the goal is not to output the final evidence directly, but to create a candidate pool with high gold-evidence coverage. We therefore tune preprocessing and BM25 parameters by recall@500. The value 500 is used as a recall-oriented but still tractable pool size: it is large enough to include many gold passages that lexical ranking misses at top-5, but small enough for cross-encoder reranking to remain feasible because reranking cost grows linearly with the number of claim--evidence pairs. After this broad recall stage, evidence selection is tuned by final evidence F-score, and classifier settings are selected by development accuracy together with per-class behaviour.

We examine class imbalance through weighted cross-entropy. For a class $y$ with training count $n_y$, the unnormalised weight is $(N/n_y)^p$, where $p=0$ gives no weighting, $p=0.5$ gives square-root inverse-frequency weighting, and $p=1$ gives full inverse-frequency weighting. The weights are then renormalised so that their sum equals the number of labels. We report all three settings because the highest overall accuracy and the best minority-class recall do not occur under the same weighting scheme.
...
This result motivates why the final pipeline uses a broad but bounded candidate pool. Top-5 is too narrow because it misses many gold passages, while top-500 gives the reranker a much larger opportunity to recover relevant evidence. Top-1000 further increases raw recall from 0.6179 to 0.6949, so it confirms that some additional gold evidence remains beyond rank 500. However, this gain comes with lower direct F1 and would double the number of cross-encoder forward passes in the reranking stage. We therefore use top-500 as a practical compromise: it provides much higher recall coverage than small candidate pools while keeping MiniLM reranking tractable in Colab. The later MiniLM reranker and relative-$\delta$ selector then reduce this broad candidate pool to about four final evidence passages per claim.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\columnwidth]{error_analysis.png}
