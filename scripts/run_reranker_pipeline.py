"""Run BM25 + cross-encoder reranker + majority classifier.

This is Phase 3's fixed-K pipeline. Classification remains majority-class on
purpose; the goal here is to isolate retrieval improvement.
"""
...
LOGGER = get_logger("run_reranker_pipeline")
...
def main() -> None:
    ...
    output_path = args.output or (
        CONFIG.paths.outputs / f"{args.split}-reranker-predictions-k{args.top_k}.json"
    )
    ...
    LOGGER.info("Loading reranker from %s", args.model_dir)
    ...
    LOGGER.info("Majority label = %s", classifier.label.value)

    claims = load_claims(claims_path)
    LOGGER.info(
        "Running reranker pipeline: split=%s, claims=%d, candidate_k=%d, top_k=%d",
        args.split,
        len(claims),
        args.candidate_k,
        args.top_k,
